    ...
    <div class="card-body">
      ...
      <form action="{{route('produk.store')}}" method="POST" enctype="multipart/form-data"> {{--//postnya badag!--}}
        ...
        <div class="form-group">
          ...
          <textarea name="deskripsi" cols="117" class="form-control" rows="10" value="{{$data->id}}" {{$data->id == $produk->deskripsi ? 'selected' : '' }}></textarea>
          @endforeach
        </div>
        <div class="form-group">
          <label for="exampleInputName1">Stok Produk</label>
          <input type="number" name="stok_produk" class="form-control" value="{{ $produk->stok_produk }}">
        </div>
        <div class="form-group">
